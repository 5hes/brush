...
impl<'a> WordExpander<'a> {
    ...
    async fn expand_word_piece(
        ...
        let expansion: Expansion = match word_piece {
            ...
            brush_parser::word::WordPiece::TildePrefix(option) => Expansion::from(
                ExpansionPiece::Unsplittable(self.expand_tilde_expression(&option)?),
            ),
    ...
    fn expand_tilde_expression(
        &self,
        option: &brush_parser::word::TildeOption,
    ) -> Result<String, error::Error> {
        match option {
            brush_parser::word::TildeOption::Home => {
            ...
            brush_parser::word::TildeOption::UserHome(username) => {
                Ok(sys::users::get_user_home_dir(username).map_or_else(
                    || std::format!("~{username}"),
                    |p| p.to_string_lossy().to_string(),
                ))
            }
            brush_parser::word::TildeOption::WorkingDir => {
                Ok(self.shell.working_dir.to_string_lossy().to_string())
            }
            brush_parser::word::TildeOption::OldWorkingDir => {
                if let Some(old_pwd) = self.shell.env.get_str("OLDPWD") {
                    Ok(old_pwd.to_string())
                } else {
                    Ok(String::from("~-"))
                }
            }
            brush_parser::word::TildeOption::NthDirInDirStack(n, plus_used) => {
                if *n == 0 {
            ...
            brush_parser::word::TildeOption::NthDirFromEndOfDirStack(n) => {
